...
export function saveCredentials(creds: StoredCredentials): void {
  ...
  // recursive mkdir is idempotent, so no check-then-create race.
...
export function loadCredentials(): StoredCredentials | null {
  const path = getCredentialsPath();
  try {
    // Read directly and treat a missing file as "not logged in" instead of
    // checking existence first (which would leave a check-then-use race).
